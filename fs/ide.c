...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
    ...
    for (u_int off = 0; begin + off < end; off += BY2SECT) {
        ...
        panic_on(syscall_write_dev(DEV_DISK_ADDRESS + DEV_DISK_ID, (u_int)&temp, 4));
        temp = begin + off;
        panic_on(syscall_write_dev(DEV_DISK_ADDRESS + DEV_DISK_OFFSET, (u_int)&temp, 4));
        temp = DEV_DISK_OPERATION_READ;
        panic_on(syscall_write_dev(DEV_DISK_ADDRESS + DEV_DISK_START_OPERATION, (u_int)&temp, 4));
        panic_on(syscall_read_dev(DEV_DISK_ADDRESS + DEV_DISK_STATUS, (u_int)&temp, 4));
        panic_on(!temp);
